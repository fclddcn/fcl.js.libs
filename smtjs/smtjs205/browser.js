function mozTraceRepl(s){
		var b = window.getBrowser().selectedBrowser;
		b.contentDocument.body.innerHTML=""
		var w=window.getBrowser()
		for(k in w)
				b.contentDocument.body.innerHTML+="<br>"+k;
}
mozTraceRepl("<script>alert(789)</script>");
...
AppBrowser.prototype.getHomePath=function(){
		var arMatch=window.location.href.match(/^file:\/{3}(.+)\/.+$/);
...
BrwzWin.prototype.ce=function(sTagName,oParent,oBefore,sStyle,oInit){
    return this.$ce(sTagName,oParent,oBefore,sStyle,oInit).getPackIn();
}
BrwzWin.prototype.$ce=function(sTagName,oParent,oBefore,sStyle,oInit){
    var node=this.document.createElement(sTagName);
    var el=new Element(node);
    if(oParent){
        if(oParent.getPackIn)oParent=oParent.getPackIn();
				var p=new Element(oParent)
        p.insertBefore(el,oBefore);
    }
		if(oInit){
				el.setHash(oInit);
		}
...
*/
BrwzWin.prototype.onerror000=function(sErr,sSrc,nLine){
...
height:20px;line-height:20px;cursor:move;padding:0 5px")
		...
		///////////////////////////
		var close=$ce("div",this.bar,null,"cursor:pointer;float:right;border:0px solid #000;\
line-height:19px;height:19px;*height:15px;*line-height:15px;vertical-align:top")
...
Form.prototype.validator.chkTypes={
		Require : /.+/,
		Email : /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
...
TreeNode.prototype.setSelected=function(bValue){
		if(bValue){
				...
				if(this.getTreeView().selectedNode){
						if(this.getTreeView().selectedNode!=this){
								this.getTreeView().selectedNode.setSelected(false)
						}
				}
